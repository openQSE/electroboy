...
class ServiceTests(unittest.TestCase):
    ...
    def test_pane_layout_allows_independent_duplicate_types(self) -> None:
        ...
        split_resize = read_service_text_asset("js/core/split-resize.js")
        ...
        self.assertIn("nodeVisible(candidate) {", runtime)
        self.assertIn('!element.hidden && !element.closest("[hidden]")', runtime)
        self.assertIn("afterUpdate: fitTerminal,", runtime)
        self.assertIn("function branchIsVisible(node, options)", split_resize)
        self.assertIn("nodeVisible: options.nodeVisible,", split_resize)
    ...
    def test_pane_window_supports_persistent_split_workspaces(self) -> None:
        ...
        self.assertIn('parameters.set("artifact", "empty");', page)
        self.assertIn('parameters.set("document_path", content.target.path);', page)
        ...
        self.assertIn('data.type === "electroboy:pane-artifact-change"', workspace)
        self.assertIn("item.content = cloneContent(data.item);", workspace)
        ...
        self.assertIn("global.ElectroBoyStatefulDOM?.collapseSplit(", workspace)
        ...
        self.assertIn("function renderIncrementalClose(id)", workspace)
        self.assertIn("if (!renderIncrementalClose(id))", workspace)
        self.assertIn("function renderIncrementalLeaf(item)", workspace)
    ...
    def test_editable_mind_map_workspace_has_keyboard_canvas_and_links(self) -> None:
        ...
        self.assertIn("function layoutPositionSnapshot()", page)
        self.assertIn('if (mode !== "freeform") {', page)
        self.assertIn("childrenOf(null).forEach((root) => applyLayoutMode(root));", page)
        self.assertIn("layoutPositionSnapshot() !== beforeLayout", page)
    ...
    def test_generic_corkboard_renders_database_record_cards(self) -> None:
        ...
        self.assertIn("function resolveSaveTheCatLayout", page)
        self.assertIn("function renderSaveTheCatStructure", page)
        self.assertIn('className = "save-the-cat-row"', page)
        self.assertIn('"structural-gap"', page)
